...
async function getBrowserVersion() {
  const driver = await new Builder().forBrowser("chrome").setChromeOptions(new Options().addArguments("--headless")).build();
...
async function createWebDriver(options) {
  const driver = await new Builder().forBrowser("chrome").setChromeOptions(options || new Options()).build();
  return driver;
}

// src/download-extension/download-extension.ts
import fs from "fs";
import { Options as Options2 } from "selenium-webdriver/chrome";
...
function addExtension(options) {
  if (options)
    ...
    return new Options2().addExtensions(PATH_TO_EXTENSION);
}
